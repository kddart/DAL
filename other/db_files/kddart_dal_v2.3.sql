...
CREATE TABLE `designtype` (
  ...
  `DesignTemplateFile` VARCHAR(255) NULL COMMENT 'The template file defines how the parameter are required to be inserted in the input file for the design software.',
...
CREATE TABLE `generaltype` (
  ...
  `Class` SET('site','item','container','deviceregister','trial','trialevent','sample','specimengroup','specimengroupstatus','state','parent','itemparent','genotypespecimen','markerdataset','workflow','project','itemlog','plate','genmap','multimedia','tissue','genotypealias','genparent','genotypealiasstatus','traitgroup','unittype','multilocation','breedingmethod') NOT NULL COMMENT 'class of type - possible values (site, item, container, deviceregister, trial, operation, sample, specimengroup, specimengroupstatus, state, parent, itemparent, genotypespecimen, markerdataset, workflow, project, itemlog, plate, genmap, multimedia, tissue, genotypealias, genparent, genotypealiasstatus, traitgroup, unittype, multilocation, breedingmethod)',
...
-- table multiloctrial
CREATE TABLE `multiloctrial` (
  `MultiLocTrialId` INTEGER NOT NULL AUTO_INCREMENT COMMENT 'Internal MLT id',
  `MultiLocTrialName` VARCHAR(254) NOT NULL COMMENT 'MLT name',
  `MultiLocTrialOwner` INTEGER NOT NULL COMMENT 'User Id - who is the owner of the MLT',
  `MultiLocTrialType` INTEGER NOT NULL COMMENT 'Generaltype id for a class multilocation',
  `MultiLocTrialStart` DATE NULL COMMENT 'MLT start date',
  `MultiLocTrialEnd` DATE NULL COMMENT 'MLT end date',
  `MultiLocTrialNote` TEXT NULL COMMENT 'Long description of MLT',
  PRIMARY KEY(`MultiLocTrialId`)
) ENGINE=InnoDB COMMENT='Multi location trial grouping. Arbitrary grouping of trials, can be used e.g. for multienvironmental trials or group of trials in multiyear experiment, which share the design.';

CREATE UNIQUE INDEX `xmlt_Name` ON `multiloctrial` (`MultiLocTrialName`);

CREATE  INDEX `xmlt_Owner` ON `multiloctrial` (`MultiLocTrialOwner`);

CREATE  INDEX `xmlt_Type` ON `multiloctrial` (`MultiLocTrialType`);

CREATE  INDEX `xmlt_start` ON `multiloctrial` (`MultiLocTrialStart`);

CREATE  INDEX `xmlt_end` ON `multiloctrial` (`MultiLocTrialEnd`);

ALTER TABLE `multiloctrial` CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;

-- table multiloctrialentry
CREATE TABLE `multiloctrialentry` (
  `MultiLocTrialEntryId` INTEGER NOT NULL AUTO_INCREMENT COMMENT '',
  `MultiLocTrialId` INTEGER NOT NULL COMMENT '',
  `TrialId` INTEGER NOT NULL COMMENT '',
  PRIMARY KEY(`MultiLocTrialEntryId`)
) ENGINE=InnoDB COMMENT='';

CREATE  INDEX `xmlte_mltid` ON `multiloctrialentry` (`MultiLocTrialId`);

CREATE  INDEX `xmlte_trialid` ON `multiloctrialentry` (`TrialId`);

ALTER TABLE `multiloctrialentry` CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;

-- table multiloctrialfactor
CREATE TABLE `multiloctrialfactor` (
  `MultiLocTrialId` INTEGER NOT NULL COMMENT '',
  `FactorId` INTEGER NOT NULL COMMENT '',
  `FactorValue` VARCHAR(254) NOT NULL COMMENT '',
  PRIMARY KEY(`MultiLocTrialId`, `FactorId`)
) ENGINE=InnoDB COMMENT='Factor columns for multi location trials';

CREATE  INDEX `FactorId` ON `multiloctrialfactor` (`FactorId`);

ALTER TABLE `multiloctrialfactor` CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci;
...
ALTER TABLE `multiloctrialfactor` ADD FOREIGN KEY (`MultiLocTrialId`)
  REFERENCES `multiloctrial` (`MultiLocTrialId`) 
  ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE `multiloctrialfactor` ADD FOREIGN KEY (`FactorId`)
  REFERENCES `factor` (`FactorId`) 
  ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE `multiloctrialentry` ADD FOREIGN KEY (`MultiLocTrialId`)
  REFERENCES `multiloctrial` (`MultiLocTrialId`) 
  ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE `multiloctrialentry` ADD FOREIGN KEY (`TrialId`)
  REFERENCES `trial` (`TrialId`) 
  ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE `multiloctrial` ADD FOREIGN KEY (`MultiLocTrialType`)
  REFERENCES `generaltype` (`TypeId`) 
  ON DELETE NO ACTION ON UPDATE NO ACTION;

ALTER TABLE `multiloctrial` ADD FOREIGN KEY (`MultiLocTrialOwner`)
  REFERENCES `systemuser` (`UserId`) 
  ON DELETE NO ACTION ON UPDATE NO ACTION;
