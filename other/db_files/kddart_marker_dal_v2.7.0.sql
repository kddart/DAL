...
-- input file: ER_dbmodel_Marker-2.7.0.xml
...
ALTER TABLE "extractdata" ADD FOREIGN KEY ("AnalGroupExtractId")
...
('analysisgroup', 'DateCreated', 'creation time of the analysis group', 0),
('analysisgroup', 'DateUpdated', 'last update date time of the analysis group', 0),
...
('dataset', 'DateCreated', 'date time when dataset was created', 0),
('dataset', 'DateUpdated', 'date time when dataset was last updated', 0),
...
('extractdata', 'ExtractDataId', 'Internal extract data id', 1),
('extractdata', 'AnalGroupExtractId', 'Extract in analysis group for which data are being imported', 0),
('extractdata', 'OperatorId', '(FK) system user who created the record', 0),
('extractdata', 'DateCreated', 'date when record was created', 0),
('extractdata', 'ExternalId', 'external identification - could be target id from DArT or any other unique identifier of extract sample as the reference to the external source', 0),
('extractdata', 'ExtractDataMeta', 'meta information about extract data record - json object', 0),
('extractdatafile', 'ExtractDataFileId', 'internal extract data file id', 1),
('extractdatafile', 'ExtractDataId', 'extract data record that file refers to', 0),
('extractdatafile', 'OperatorId', '(FK) operator id - who uploaded or created the file', 0),
('extractdatafile', 'FileType', '(FK) file type - should be a controlled set of known file types from general type - to start with - fastq, fastqcol, fasta, vcf', 0),
('extractdatafile', 'UploadTime', 'time of the file upload', 0),
('extractdatafile', 'FileExtension', 'file extension', 0),
('extractdatafile', 'FileMeta', 'meta data about the file - json object storing potentially file size, md5 sum, compression algorithm, etc.', 0);
