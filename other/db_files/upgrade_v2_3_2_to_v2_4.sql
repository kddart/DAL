...
ALTER TABLE activitylog ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARACTER SET utf8 comment='A log of system user activities';

ALTER TABLE authorisedsystemgroup ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARACTER SET utf8 comment='List of users in system group.';
...
ALTER TABLE contact ENGINE=InnoDB AUTO_INCREMENT=20 DEFAULT CHARACTER SET utf8 comment='List of general contacts (not only system users, but also collaborators, material providers, site managers, etc). Links with organisation.';

ALTER TABLE crossing ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARACTER SET utf8 comment='A table which stores crossing information for nursery management';

ALTER TABLE designtype ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARACTER SET utf8 comment='Each trial (experiment) has a design (layout/method) describing how the genotypes have been planted, the number of replicates, etc. Design type identifies the type of a trial design. For example: - Randomised Block using DiGGer - The trial design of this design type is generated by the DiGGer. - Unknown Design Type. ';

ALTER TABLE deviceregister ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARACTER SET utf8 comment='Register of devices, which have access to the database to supply data. Devices to include those that supply environmental data or seed inventory data (e.g. scales) and extend to those that supply for phenotypic measurements.  One device can record more than one data type / parameter to more than one environment layer.';

ALTER TABLE factor ENGINE=InnoDB AUTO_INCREMENT=22 DEFAULT CHARACTER SET utf8 comment='Main database table containing the definitions of all virtual columns.  A uniform mechanism across the database where all tables with the suffix factor relate to this table.';
...
ALTER TABLE keyword ADD COLUMN OperatorId integer(11) NULL DEFAULT NULL comment 'optional owner of the keyword',
                    ...
                    ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARACTER SET utf8 comment='System wide collection of keywords';
...
ALTER TABLE keywordgroup DROP INDEX kwg_KeywordGroupName,
                         ...
                         ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARACTER SET utf8 comment='Arbitrary groups of keywords for easy search or other purpose.';

ALTER TABLE keywordgroupentry ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARACTER SET utf8 comment='list of keywords in the keyword group(s)';

ALTER TABLE multimedia ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARACTER SET utf8 comment='This table stores references for multimedia files stored in KDDart system. Files will be attached to particular records in other tables, to e.g. illustrate processes, events or document.';

ALTER TABLE organisation ENGINE=InnoDB AUTO_INCREMENT=20 DEFAULT CHARACTER SET utf8 comment='List of collaborating and internal and/or external organisations. ';

ALTER TABLE pedigree ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARACTER SET utf8 comment='Table, which defines specimen (genotype) pedigrees (genealogy). This design assumes that one specimen may have more than 2 parents, which is never a case in reality, but in some scenarios (e.g. bulk pollination)  this kind of information may need to be preserved.';

ALTER TABLE project ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARACTER SET utf8 comment='A very generic table to enable the grouping of trials according to some organisational distinction.  Projects may have very different requirements therefore it maybe more appropriate to use the factor column to standardise data within the organisation.';
...
ALTER TABLE specimengroup ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARACTER SET utf8 comment='Defines a name for a group of specimens. Possible applications can be: many plants which went through selfing process; groups for next years testing; groups for genotyping.';

ALTER TABLE specimengroupentry ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARACTER SET utf8 comment='Maintains the many to many relationship between specimens and specimens groups. ';

ALTER TABLE specimenkeyword ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARACTER SET utf8 comment='keywords for a specimen';

ALTER TABLE storage ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARACTER SET utf8 comment='General storage locations in a tree like hierarchy. Allows construction of locations such as: - Building 1 - Room 7 - Rack 5  - Shelf 299.';

ALTER TABLE systemgroup ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARACTER SET utf8 comment='Definitions of system groups. Most important in setting record level privileges for many entities.';
...
ALTER TABLE trait ADD COLUMN TraitLevel set('trialunit', 'subtrialunit', 'notetrialunit') NOT NULL DEFAULT 'trialunit' comment 'level at which trait is being used (scored), additional global distinction',
                  ...
                  ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARACTER SET utf8 comment='Defines the measurement values for the genotype and trial. The specification of a validation rule is optional, however the format must adhere to either a regular or boolean expression. For example a validation rule could be: - Regular Expression TraitValRule=([A-Z]*) - Boolean Expression TraitValRule=(xg1 and xk50) ';

ALTER TABLE traitalias ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARACTER SET utf8 comment='List of alias names for traits. Sometime for e.g. multinational organisations the same trait can be translated into several languages.  The TraitLang column provides standard language code for translation interfaces. ';

ALTER TABLE treatment ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARACTER SET utf8 comment='Description (or value) for a treatment for trial unit (plot). Useful if trying to measure response to certain level of irrigations, fertilisation, etc.';

ALTER TABLE trial ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARACTER SET utf8 comment='This table contains list of trials (equivalent to field experiments) performed within sites. ';

ALTER TABLE trialdimension ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARACTER SET utf8 comment='Optional definition of the trial dimensionality. If user wants to define that X dimension is called e.g. row and Y dimension is called column - this is a place to do it. In the future this can be extended to contain trial level information about e.g. plot width, plot height, plot spaces, etc. Trial and dimention combination has to be unique - forced by unique index.';

ALTER TABLE trialevent ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARACTER SET utf8 comment='Trial events are additional descriptors for operations (events) affecting an entire trial.  Example events:  - spray nitrogen 10 tons,  - rain shed over trial during flowering season,  - catastrophic rainfall or thunder, etc.';

ALTER TABLE trialgroup ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARACTER SET utf8 comment='Multi location trial grouping. Arbitrary grouping of trials, can be used e.g. for multienvironmental trials or group of trials in multi year experiment, which may or may not share the design.';

ALTER TABLE trialgroupentry ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARACTER SET utf8 comment='Many to many link between trials and trial group';

ALTER TABLE trialtrait ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARACTER SET utf8 comment='Part of the trial definition. List of traits expected to be measured for a trial. This table should be rather treated as a control checkpoint, while inserting real measurements into samplemeasurement table.';

ALTER TABLE trialunit ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARACTER SET utf8 comment='This table stores information about each unit of the trial. It assumes that the unit is homogeneous.  One trial unit can contain more than one specimen (e.g. for hybrid plants or when one plant died and another was replanted in place).  Trial unit is a measured unit, which can be further divided by sample types distinguished within.';

ALTER TABLE trialunitkeyword ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARACTER SET utf8 comment='keywords for a trial unit';

ALTER TABLE trialunitspecimen ENGINE=InnoDB AUTO_INCREMENT=54 DEFAULT CHARACTER SET utf8 comment='Maintains the many to many relationship between trialunit and specimens. If specimen is the representation of the whole genotype (or composition of genotypes), it will have a single record here. Allows the sampling of multiple specimens from a unit.  For instances of long lived trials (e.g. tree living 30 years) this enables the reassignment of the same plant into different trial units.';

ALTER TABLE workflow ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARACTER SET utf8 comment='Contains workflows used in the system for trial workflows and can also be used to link to other processes.  Caters for the definition of multiple workflows for different situations.';
