...
<xsl:template match="DATA">#CrossingId,SampleTypeId,TraitId,MeasurementDateTime,InstanceNumber,TraitValue
<xsl:apply-templates select="ROW"/>
</xsl:template>

<xsl:template match="ROW">
  <xsl:value-of select="CrossingId/@Value"/><xsl:text>,</xsl:text>
